...
#    startTime=$(date +%s.%N)
#    endTime=$(date +%s.%N)
#    echo "$(getElapseTime ${startTime} ${endTime})" >> ${timeFile}
function getElapseTime() {
    ...
    declare -i local time=$(( ( 10#$end_s - 10#$start_s ) * 1000 + ( 10#$end_ns / 1000000 - 10#$start_ns / 1000000 ) ))
    echo "${time}ms"
}
...
function usage() {
    ...
    echo "  -r FILE: analyze FILE"
    echo "  -d DIRECTORY: analyze FILES under DIRECTORY"
    echo "  -s: get the statistical information"
    echo "  -o OUTPUT-FILE: output the analyzing result into this file"
...
function isLineValid() {
    ...
    echo "isLineValid: $(getElapseTime ${startTime} ${endTime})" >> ${timeFile}
    return 0
}
...
do
    let "numberArray[${index}]=0"
    let "percentArray[${index}]=0"
    fileArray[${index}]=""
done

if [ "$outputFile" == "" ]; then
    outputFile="power_statistics_$(date +%Y%m%d).txt"
fi
outputFileDetail="${outputFile}.detail"
echo -ne "Name: ${outputFileDetail}\n" > ${outputFileDetail}

if [ "${fileName}" != "" ]; then
    echo "handling ${fileName}"
    getTotalString=$(getTotalInfo ${fileName})
    ...
    if [ ${section} -lt 255 ]; then
        let "numberArray[${section}]=numberArray[${section}]+1"
        if [ "${fileArray[${section}]}" == "" ]; then
            fileArray[${section}]="${fileName}"
        else
            fileArray[${section}]="${fileArray[${section}]},${fileName}"
        fi
...
if [ "${fileDirectory}" != "" ]; then
    for fileStr in $(find ${fileDirectory} -name "bugreport_*.log")
    do
        echo "handling ${fileStr} in ${fileDirectory}"
        getTotalString=$(getTotalInfo ${fileStr})
        totalTimeInt=$(echo ${getTotalString} | awk '{print $1}')
        totalBatteryInt=$(echo ${getTotalString} | awk '{print $2}')
        echo "totalTimeInt is ${totalTimeInt}"
        echo "totalBatteryInt is ${totalBatteryInt}"
        ...
        if [ ${section} -lt 255 ]; then
            let "numberArray[${section}]=numberArray[${section}]+1"
            if [ "${fileArray[${section}]}" == "" ]; then
                fileArray[${section}]="${fileStr}"
            else
                fileArray[${section}]="${fileArray[${section}]},${fileStr}"
            fi
            echo -ne "\nLogFileName: ${fileName}\nTotal Discharging Time: $(getTimeFromMs ${totalTimeInt})\n\
Total Discharging Battery Count(%): ${totalBatteryInt}\nConversion Result(In 100% Battery): $(getTimeFromMs ${statisticsResult})\n\
Total Discharging Time(ms): ${totalTimeInt}\nConversion Result(ms in 100% Battery): ${statisticsResult}\n${dischargingInfo}\n${chargingInfo}\n${screenOffInfo}\n${screenOnInfo}" >> ${outputFileDetail}
...
# Check if need to output the statistics to file
if [ "${outputStatistics}" == "false" ]; then
    exit 0
fi

declare -i totalNumber=0
for ((index=0; index<=48; index++))
do
    let "totalNumber=totalNumber+numberArray[${index}]"
done
if [ ${totalNumber} -eq 0 ]; then
    echo "NONE Statistics INFO"
    exit 0
fi
for ((index=0; index<=48; index++))
do
    let "percentArray[${index}]=numberArray[${index}]*100/totalNumber"
done
echo -ne "Power Statistics\n" > ${outputFile}
echo -ne "\nhour\t\tLogCount\t\tPercent(%)\n" >> ${outputFile}
echo -ne "<1\t\t${numberArray[0]}\t\t\t${percentArray[0]}\n" >> ${outputFile}
for ((index=1; index<=47; index++))
do
    echo -ne "${index}-$[$index+1]\t\t${numberArray[$index]}\t\t\t${percentArray[$index]}\n" >> ${outputFile}
done
echo -ne ">48\t\t${numberArray[48]}\t\t\t${percentArray[48]}\n" >> ${outputFile}
exit 0
